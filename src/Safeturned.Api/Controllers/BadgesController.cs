...
public class BadgesController : ControllerBase
{
    ...
    [HttpGet("official/status")]
    [AllowAnonymous]
    public async Task<IActionResult> GetOfficialBadgesStatus()
    {
        try
        {
            await using var scope = _serviceScopeFactory.CreateAsyncScope();
            ...
            var officialBadgeIds = OfficialBadgeConstants.All;

            var badges = await filesDb.Set<Badge>()
                .AsNoTracking()
                .Where(b => officialBadgeIds.Contains(b.Id))
                .Select(b => new
                {
                    b.Id,
                    b.Name,
                    IsVerified = b.LinkedFileHash != null,
                    b.UpdatedAt
                })
                .ToListAsync();

            var allVerified = badges.Count == 3 && badges.All(b => b.IsVerified);
        ...
        {
            _logger.Error(ex, "Error retrieving official badges status");
            return StatusCode(500, new { error = "Failed to retrieve official badges status" });
        }
    }
